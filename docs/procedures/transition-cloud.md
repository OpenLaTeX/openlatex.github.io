...
Les configurations Terraform sont dans `infra/terraform/` (dossiers par provider : `aws/`, `azure/`, `digitalocean/`).
...
En résumé :
1. Sur l'ancien serveur, lancer une sauvegarde manuelle : `/home/admin/openlatex/backend/cron/db-save/dump_db.sh`
...
```bash
cd infra/terraform/<ancien-provider>
terraform destroy
```
